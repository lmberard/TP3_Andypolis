    ...
    if (jugadores[id_jugador_actual - 1].obtener_energia() >= ENERGIA_NECESARIA_PARA_REPARAR)
    {
        ...
        if (mapa.coordenadas_validas(coordenadas))
        {
            if (mapa.obtener_edificio(coordenadas) != NULL)
            {
                ...
                {
                    msjeError("El edificio seleccionado no le pertenece.");
                }
            }
            else
            {
                msjeError("El casillero seleccionado no contiene un edificio construido.");
            }
        }
    }
...
{
    msjeOK("Se ha reparado satisfactoriamente el edificio: " + edif_ptr->obtener_nombre());
}
